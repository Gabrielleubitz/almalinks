...
async function findContactByEmail(token, email) {
  ...
  if (!res.ok) {
    ...
    console.warn('[hubspot-email-log] findContactByEmail failed', { status: res.status, email, body: errBody?.slice(0, 300) });
    return null;
  }
...
async function createContactWithEmail(token, email) {
  ...
  if (!res.ok) {
    const errBody = await res.text();
    console.warn('[hubspot-email-log] createContactWithEmail failed', { status: res.status, email, body: errBody?.slice(0, 500) });
    return null;
  }
  const data = await res.json().catch(() => ({}));
  return data.id ?? null;
