...
/** Required: pipeline and dealstage. HubSpot Deals require these. Override via env if your portal uses different IDs. */
...
function buildDealProperties(event) {
  ...
  // date (Database) -> closedate (HubSpot, ms timestamp). Send 0 if missing.
  const dateVal = e.date;
  let closedateMs = 0;
  if (dateVal != null) {
    if (typeof dateVal === 'number') closedateMs = dateVal;
    else if (typeof dateVal.toDate === 'function') closedateMs = dateVal.toDate().getTime();
    else if (typeof dateVal.toMillis === 'function') closedateMs = dateVal.toMillis();
    else closedateMs = new Date(dateVal).getTime();
  }
  ...
  // chapter (Database) -> chapter (HubSpot). Send "" if missing.
  props.chapter = (e.chapter ?? '').toString().trim();

  // attendedCount (Database) -> attended (HubSpot). Send 0 if missing.
  ...
  // zoomRecordingUrl (Database) -> zoom_recording_link (HubSpot). Send "" if missing.
  props.zoom_recording_link = (e.zoomRecordingUrl ?? e.zoom_recording_url ?? '').toString().trim();
