...
/** Convert slugified value (e.g. 'tel-aviv') to display name (e.g. 'Tel Aviv') */
...
function buildDealProperties(event) {
  ...
  // date (Database) -> date + closedate (HubSpot). Unix ms timestamp at UTC midnight. Send 0 if missing.
  ...
  props.date = Number.isFinite(dateMs) ? dateMs : 0;
  props.closedate = props.date;
  ...
  // chapter (Database) -> chapter (HubSpot enumeration). Normalize e.g. 'tel-aviv' -> 'Tel Aviv'. Send "" if missing.
  props.chapter = normalizeChapter(e.chapter ?? '') || '';
  ...
  // zoomRecordingUrl (Database) -> zoom_link (HubSpot, Zoom Recording Link). Send "" if missing.
