 ...
 * Mailchimp uses condition_type as discriminator; for EmailAddress use op + value (no field).
 * @param {string} listId
...
export async function createSegment(listId, name, conditions) {
  ...
  console.log('[mailchimp-campaign] createSegment request:', JSON.stringify({ listId, name, conditionsCount: conditions.length }));
...
export async function createCampaignWithSegment(options) {
  ...
  const subject = (subjectLine && String(subjectLine).trim()) || 'Welcome to Alma Links!';
  const from = (fromName && String(fromName).trim()) || 'Alma Links';
 ...
 * Uses MAILCHIMP_WELCOME_TEMPLATE_HTML or a simple default. Env: MAILCHIMP_REPLY_TO, MAILCHIMP_FROM_NAME, MAILCHIMP_AUDIENCE_ID.
 * @param {string} email - Recipient email (must be in audience or segment may be empty)
 * @param {string} [displayName] - Optional name for personalization
 * @returns {Promise<{ campaignId: string, segmentId: number }>}
 */
export async function sendWelcomeCampaign(email, displayName) {
  const config = getMailchimpAudienceConfig();
  if (!config) throw new Error('Mailchimp not configured (MAILCHIMP_AUDIENCE_ID and MAILCHIMP_MARKETING_API_KEY)');
  ...
  const fromName = (process.env.MAILCHIMP_FROM_NAME || 'Alma Links').trim() || 'Alma Links';
  const listId = config.audienceId;
  const userEmail = (email || '').trim().toLowerCase();
  if (!userEmail) throw new Error('email is required for welcome campaign');

  // Segment: target only this email. Static segment with condition: email_address is userEmail.
  const segmentName = `welcome-${userEmail.replace(/[^a-z0-9]/g, '-').slice(0, 50)}-${Date.now()}`;
  const conditions = [
    { condition_type: 'EmailAddress', field: 'email_address', op: 'is', value: userEmail },
  ];
  ...
  const subjectLine = 'Welcome to Alma Links!';
  ...
  // Set content BEFORE sending. Fallback HTML if no template.
  const rawHtml = (process.env.MAILCHIMP_WELCOME_TEMPLATE_HTML || '').trim();
  const html = rawHtml
    ? rawHtml.replace(/\{\{\s*name\s*\}\}/g, (displayName || '').trim() || 'there')
    : `<h1>Welcome to Alma Links!</h1><p>Hi ${escapeHtml((displayName || 'there').trim())},</p><p>Thanks for signing up. We're glad to have you.</p><p><a href="https://almalinks.org">Visit Alma Links</a></p>`;
