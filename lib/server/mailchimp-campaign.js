...
export async function createCampaignWithSegment(options) {
  ...
  const from = (fromName && String(fromName).trim()) || 'AlmaLinks';
 ...
 * @param {string} [fromName] - Defaults to MAILCHIMP_FROM_NAME or "AlmaLinks"
...
export async function createAndSendEventCampaign(event, fromName, replyTo, listId) {
  ...
  const from_name = fromName || process.env.MAILCHIMP_FROM_NAME || 'AlmaLinks';
...
export async function sendWelcomeCampaign(email, displayName, joinRequestId) {
  ...
  const fromName = (process.env.MAILCHIMP_FROM_NAME || 'AlmaLinks').trim() || 'AlmaLinks';
