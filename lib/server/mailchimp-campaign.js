 ...
 * Uses ONLY Marketing API (no Mandrill/Transactional). Campaign HTML is wrapped in shared Alma Links theme.
 *
...
export async function createAndSendEventCampaign(event, fromName, replyTo, listId) {
  ...
  `;
  const html = wrapInAlmaTheme(innerHtml, { title: subjectLine });
...
export async function sendWelcomeCampaign(email, displayName, joinRequestId) {
  ...
  // Set content BEFORE sending. Fallback HTML with APP_URL login link. Wrap in Alma theme for consistency with transactional emails.
  const baseUrl = getAppBaseUrl();
  const rawHtml = (process.env.MAILCHIMP_WELCOME_TEMPLATE_HTML || '').trim();
  const namePlaceholder = (displayName || '').trim() || 'there';
  const innerHtml = rawHtml
    ? rawHtml.replace(/\{\{\s*name\s*\}\}/g, namePlaceholder)
    : `<h1>Welcome, ${escapeHtml(namePlaceholder)}</h1><p>Your signup is received.</p><p><a href="${escapeHtml(baseUrl)}/login">Log in</a></p>`;
  const html = wrapInAlmaTheme(innerHtml, { title: 'Welcome to Alma Links!' });
