...
export async function createCampaign(options) {
  ...
  if (!res.ok) {
    const msg = data.detail || data.title || `Create campaign failed ${res.status}`;
    console.error('[mailchimp-campaign] createCampaign failed:', res.status, data);
    throw new Error(msg);
  }
  return data;
}
...
export async function setCampaignContent(campaignId, html) {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    const msg = data.detail || data.title || `Set content failed ${res.status}`;
    console.error('[mailchimp-campaign] setCampaignContent failed:', res.status, data);
    throw new Error(msg);
  }
  return data;
}
...
export async function sendCampaign(campaignId) {
  ...
  const data = await res.json().catch(() => ({}));
  const msg = data.detail || data.title || `Send failed ${res.status}`;
  console.error('[mailchimp-campaign] sendCampaign failed:', res.status, data);
  throw new Error(msg);
}
