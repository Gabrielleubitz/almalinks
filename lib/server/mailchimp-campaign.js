 ...
 * @param {{ listId: string, segmentId: number, subjectLine: string, fromName: string, replyTo: string }} options
 */
...
export async function sendWelcomeCampaign(email, displayName) {
  ...
  if (!reply_to) throw new Error('MAILCHIMP_REPLY_TO is required for campaigns');
  const from_name = process.env.MAILCHIMP_FROM_NAME || 'Alma Links';
  const listId = config.audienceId;

  const segmentName = `Welcome ${email} ${Date.now()}`;
  const conditions = [
  ...
  const rawHtml = process.env.MAILCHIMP_WELCOME_TEMPLATE_HTML;
  const html = rawHtml && rawHtml.trim()
