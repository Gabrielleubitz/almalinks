 ...
 *
 * Imports HubSpot events/activities into Firestore:
 * - Primary attempt: HubSpot **Events API (Event Analytics)** at /events/v3/events
 *   - If the portal/token doesn't have access (401/403 or other fatal errors), we fall back.
 * - Fallback: HubSpot CRM activity objects via /crm/v3/objects/{meetings,calls,notes,tasks,emails}
 *
 * Firestore:
 ...
 *     type: "event_analytics" | "meeting" | "call" | "note" | "task" | "email" | ...,
 ...
 *
 * Auth protection (same as sync-hubspot-contacts):
 * - If SYNC_SECRET is set in env, require header: x-sync-secret: <SYNC_SECRET>.
 * - Otherwise require Firebase Auth ID token for an admin user (Authorization: Bearer <idToken>).
 *
 * Env:
 * - HUBSPOT_ACCESS_TOKEN (required) – HubSpot Private App or OAuth token (server-side only).
...
async function authorize(req) {
  ...
  // When no SYNC_SECRET is configured, require Firebase Auth ID token for an admin.
  ...
  // firebase-init.js exports an initialized admin instance when configured.
