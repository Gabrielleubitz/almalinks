...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Remove user from all group chats (chat_members where userId == userIdToDelete)
    try {
      ...
      if (!chatMembersSnapshot.empty) {
        ...
        for (let i = 0; i < memberDocs.length; i += BATCH_SIZE) {
          ...
          memberDocs.slice(i, i + BATCH_SIZE).forEach((docSnap) => batch.delete(docSnap.ref));
          await batch.commit();
        }
        console.log(`✅ Removed user from ${memberDocs.length} chat(s)`);
      }
    } catch (chatErr) {
      console.warn('⚠️ Failed to remove user from chats:', chatErr.message);
    }
