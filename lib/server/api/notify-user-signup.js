 ...
 * From name: TRANSACTIONAL_FROM_NAME, then MAILCHIMP_FROM_NAME (see email-config getFromName).
 */
...
import { getFromEmail, getFromName, getLoginLink, getEventsLink, getAppBaseUrl, getTestRecipient } from '../email-config.js';
...
— AlmaLinks Team`;
    ...
    // Send email via transactional-email (Mailjet if configured, else Mandrill); theme applied automatically
    let messageId = null;
    let sendStatus = null;
    ...
    try {
      ...
      sendStatus = sendResult.status || (sendResult.ok ? 'sent' : 'rejected');

      if (sendResult.ok) {
        emailStatus = 'success';
      } else {
        emailStatus = 'error';
        errorMessage = sendResult.rejectReason || sendResult.error || sendStatus;
      }
