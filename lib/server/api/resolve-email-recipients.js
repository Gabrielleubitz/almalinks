...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Parse request body
    const { mode, ids, groupId, eventId, eventIds, chatId, chatIds, location, locations } = req.body;
    ...
    try {
      switch (mode) {
        ...
        case 'event': {
          ...
          if (!evIds.length) {
            return res.status(400).json({ ok: false, error: 'Select at least one event for event mode' });
          }
          const mapEv = new Map();
        ...
        case 'chat': {
          ...
          if (!cIds.length) {
            return res.status(400).json({ ok: false, error: 'Select at least one chat for chat mode' });
          }
          const mapCh = new Map();
        ...
        case 'location': {
          ...
          if (!locs.length) {
            return res.status(400).json({ ok: false, error: 'Select at least one location for location mode' });
          }
