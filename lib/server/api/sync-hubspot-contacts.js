...
export async function syncHubspotContactsToFirestore(options) {
  ...
  let deletedUsers = 0;
  if (fullResync) {
    const usersSnap = await usersCol.where('importedFrom', '==', 'hubspot').select('role', 'hubspotContactId', 'hubspotId').get();
    ...
    // Only run the expensive dedup scan when explicitly requested (not on every routine sync).
