...
export async function syncHubspotContactsToFirestore(options) {
  ...
  do {
    ...
    for (const contact of results) {
      ...
      const email = normalizeEmail(props.email?.value ?? props.email ?? '');
      ...
      if (email) {
        ...
        batch.set(
          usersCol.doc(uid),
          {
            ...
            // New canonical field name (used across the app)
            hubspotContactId: docId,
            hubspotLastSyncedAt: userNow,
            hubspotSyncStatus: 'ok',
            hubspotSyncError: null,

            // Legacy field name kept for backwards compatibility (some older tooling)
            hubspotId: docId,
