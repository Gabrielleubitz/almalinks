...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await sendWelcomeCampaign(email, finalDisplayName, joinRequestId);
    ...
    const fullBody = responseBody != null && typeof responseBody === 'object' ? responseBody : (responseBody ? { raw: String(responseBody) } : {});
    const errorsArray = fullBody.errors || fullBody.detail?.errors || (Array.isArray(fullBody) ? fullBody : []);
    const errMsg = err?.message || 'Failed to send welcome email';

    console.error('[welcome-email] Mailchimp welcome email error — full response body:', JSON.stringify(fullBody));
    console.error('[welcome-email]', errMsg);

    return res.status(500).json({
      ok: false,
      error: fullBody.title || fullBody.detail?.message || errMsg,
      code: fullBody.type === 'https://mailchimp.com/developer/marketing/docs/errors/' ? 'MAILCHIMP_VALIDATION_ERROR' : undefined,
      details: errorsArray.length ? errorsArray : (fullBody.detail ? [fullBody.detail] : undefined),
    });
  }
}
