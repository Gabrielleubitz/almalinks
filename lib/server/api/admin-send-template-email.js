 ...
 * Body: { "to": "email@domain.com", "template": "welcome" | "test" | ... | "application-follow-up" }
...
import {
  ...
  eventReminder,
  eventThankYouCheckedIn,
  applicationIntroFollowUp,
  passwordReset,
  userCredentials,
...
const TEMPLATES = {
  ...
  'welcome-approved': {
    ...
    buildHtml: (baseUrl, to) => {
      ...
      const setupPasswordLink = `${baseUrl}/reset-password?token=sample-token`;
      return welcomeApproved(name, getLoginLink(), getEventsLink(), setupPasswordLink);
    },
  },
  rejection: {
  ...
  'event-thank-you-attending': {
    subject: 'Thank you for attending: Sample Event',
    buildHtml: (baseUrl, to) => {
      const displayName = (to && to.split('@')[0]) || 'there';
      const eventName = 'Sample Event';
      const reviewUrl = `${baseUrl}/events/sample?review=1`;
      return eventThankYouCheckedIn(displayName, eventName, reviewUrl);
    },
  },
  'application-follow-up': {
    subject: 'Thank you for your application to AlmaLinks!',
    buildHtml: (baseUrl, to) => {
      const firstName = (to && to.split('@')[0]) || '';
      return applicationIntroFollowUp(firstName, baseUrl);
    },
  },
  'password-reset': {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { isAdmin, detail: adminDetail } = await resolveAdminAccess(decoded, db);
    if (!isAdmin) {
      return res.status(403).json({ ok: false, error: 'Admin required', detail: adminDetail });
    }
