...
export default async function handler(req, res) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'delete-all-activity':
        return await deleteAllActivity(req, res, currentUserId);
      default:
        return res.status(400).json({ 
          success: false, 
          error: `Unknown action: ${action}. Available actions: get-activities, get-activity-stats, cleanup-old-logs, cleanup-duplicates, log-activity, delete-all-activity` 
        });
    }
  } catch (error) {
...
// Delete all activity logs (admin only) - keeps Firebase clean
async function deleteAllActivity(req, res, adminId) {
  try {
    console.log('🗑️ Admin requested delete all activity logs');

    const db = getDb();
    const BATCH_SIZE = 500; // Firestore batch limit
    let totalDeleted = 0;

    // Delete in batches until no documents remain
    while (true) {
      ...
      console.log(`🗑️ Deleted batch of ${snapshot.docs.length} (${totalDeleted} total)`);
    }

    console.log(`✅ Delete all activity complete: ${totalDeleted} documents removed`);

    return res.status(200).json({
      success: true,
      message: `Deleted all activity logs (${totalDeleted} documents)`,
      deletedCount: totalDeleted
    });
  } catch (error) {
    console.error('❌ Error deleting all activity:', error);
    return res.status(500).json({
      success: false,
      error: error.message || 'Failed to delete all activity',
      details: process.env.NODE_ENV === 'development' ? error.stack : undefined
    });
  }
}
