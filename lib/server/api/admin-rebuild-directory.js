...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Use .select() to only fetch the fields we need — avoids downloading huge
    // hubspotContactProperties maps (~150 keys each) for every user document.
    const usersSnap = await db.collection('users').select(
      'displayName', 'name', 'work', 'title', 'bioTitle',
      'position', 'profileImage', 'avatarUrl',
      'hubspotContactProperties',
      'discoverability', 'profileVisibility'
    ).get();
    ...
    // Commit in batches of 200 to stay well under the 500-operation limit and
    ...
    for (const userDoc of usersSnap.docs) {
      try {
        ...
        if (batchCount >= BATCH_SIZE) {
          ...
          batch = db.batch();
          batchCount = 0;
          // Small pause between batches to avoid burst write rate limits
