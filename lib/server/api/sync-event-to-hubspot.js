...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Pull chapter, zoom, pictures from event doc first (may be on main doc)
    ...
    eventData.picturesUrl = (raw.picturesUrl ?? raw.pictures_url ?? eventData.picturesUrl ?? '').toString().trim();

    // Fetch privateDetails (overrides with locationText, meetingUrl, zoom_link, zoom_password, pictures_link)
    try {
      ...
      if (privateSnap.exists) {
        ...
        const privPics = (priv.picturesUrl ?? priv.pictures_url ?? '').toString().trim();
        if (privZoom) eventData.zoomRecordingUrl = privZoom;
        if (privPw) eventData.zoomPassword = privPw;
        if (privPics) eventData.picturesUrl = privPics;
      }
    } catch (privErr) {
