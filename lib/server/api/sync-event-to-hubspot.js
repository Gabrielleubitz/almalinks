...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Fetch privateDetails (locationText, meetingUrl, zoom fields, etc.) for full mapping
    try {
      ...
      if (privateSnap.exists) {
        ...
        eventData.zoomRecordingUrl = priv.zoomRecordingUrl || priv.zoom_recording_url || eventData.zoomRecordingUrl || eventData.zoom_recording_url || '';
        eventData.zoomPassword = priv.zoomPassword || priv.zoom_password || eventData.zoomPassword || eventData.zoom_password || '';
        eventData.picturesUrl = priv.picturesUrl || priv.pictures_url || eventData.picturesUrl || eventData.pictures_url || '';
      }
    } catch (privErr) {
