 ...
 *
 * Updates event private details (locationText, venueAddress, meetingUrl, resource links, zoom)
 * using Admin SDK. Bypasses client Firestore rules that may reject the write.
 *
 * Body: { eventId, locationText?, venueAddress?, meetingUrl?, resourceLinkUrl?, resourceLinkLabel?, zoomRecordingUrl?, zoomPassword? }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const data = {
      ...
      venueAddress: (req.body?.venueAddress ?? '').toString().trim() || null,
