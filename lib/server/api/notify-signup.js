...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Always include the communications inbox even if the env list forgets it.
    // Operations explicitly asked for new-application alerts to land there.
    const COMMS_EMAIL = (process.env.COMMUNICATIONS_NOTIFICATION_EMAIL || 'communications@almalinks.org').trim();

    let adminEmails = parseAdminEmails(adminEmailsEnv);
    if (COMMS_EMAIL && !adminEmails.some(e => e.toLowerCase() === COMMS_EMAIL.toLowerCase())) {
      adminEmails.push(COMMS_EMAIL);
    }
    ...
    if (adminEmails.length === 0) {
      console.warn('⚠️ No valid admin emails found (ADMIN_NOTIFICATION_EMAILS empty and comms address invalid)');
      return res.status(200).json({
        ok: true,
        skipped: true,
...
Review and approve/reject: ${pendingLink}`;
    ...
    const bodyHtml = `
<p style="margin:0 0 16px 0; font-size:16px; line-height:1.5; color:#1C1C1C;">A new user has requested to join AlmaLinks.</p>
