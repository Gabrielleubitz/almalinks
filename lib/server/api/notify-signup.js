import { getAdminPendingLink, getAppBaseUrl } from '../email-config.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let adminEmails = await getAdminNotificationEmails('signup_admin');
    if (adminEmails.length === 0) {
      console.warn('⚠️ No valid admin emails found (no Firestore admins, ADMIN_NOTIFICATION_EMAILS empty, and comms address invalid)');
      return res.status(200).json({
        ok: true,
        skipped: true,
