...
const ALLOWED_TOP_LEVEL = new Set([
  ...
  // Profile picture: allows picture URL changes to propagate to HubSpot
  'profileImage', 'avatarUrl', 'profileImagePublicId',
]);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (updates.name !== undefined && merged.fullName === undefined) {
    ...
    // Keep work/title aligned — dashboard uses `work` for job title; admin/profile-edit uses `title`.
    if (updates.work !== undefined && updates.title === undefined) {
      merged.title = merged.title || merged.work;
    } else if (updates.title !== undefined && updates.work === undefined) {
      merged.work = merged.work || merged.title;
    }
    ...
    // Only sync approved members to HubSpot — pending/rejected applicants must not appear there
    if (current.status && current.status !== 'approved') {
      // Still save to Firestore; just skip HubSpot
      await userRef.set(merged, { merge: true });
      return res.status(200).json({ ok: true, hubspotSync: false, reason: `User status is "${current.status}" — skipping HubSpot sync` });
    }
