...
// Validates audience and records intent; emails are handled via Mailchimp Marketing API / automations.
// Requires authentication (admin). Does NOT send emails directly.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Optional: sync recipients to Mailchimp audience (Marketing API only; no sending)
    try {
    ...
    console.log('[send-bulk-email] Validated', recipients.length, 'recipients (mode:', mode + '); emails via Mailchimp automation');

    // Audit: selection with explicit defaults (no undefined); deepRemoveUndefined for whole payload
    const selection = {
