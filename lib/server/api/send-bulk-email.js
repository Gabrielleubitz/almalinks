...
const MAX_STORED_RECIPIENTS = 250;
...
async function resolveEventNames(eventIds) {
  if (!eventIds.length || !db) return [];
  const names = [];
  for (const eventId of eventIds) {
    try {
      const snap = await db.collection('events').doc(eventId).get();
      names.push(snap.exists ? (snap.data()?.name || eventId) : eventId);
    } catch {
      names.push(eventId);
    }
  }
...
async function resolveChatNames(chatIds) {
  if (!chatIds.length || !db) return [];
  const names = [];
  for (const chatId of chatIds) {
    try {
      const snap = await db.collection('chats').doc(chatId).get();
      names.push(snap.exists ? (snap.data()?.name || chatId) : chatId);
    } catch {
      names.push(chatId);
    }
  }
...
async function buildAudienceReason(mode, audienceLike, recipients) {
  const count = recipients.length;
  const countLabel = `${count} recipient${count === 1 ? '' : 's'}`;

  switch (mode) {
    case 'individuals':
      if (Array.isArray(audienceLike.emails) && audienceLike.emails.length > 0) {
        return `Manual email list (${countLabel})`;
      }
      if (Array.isArray(audienceLike.ids) && audienceLike.ids.length > 0) {
        return `Selected members (${countLabel})`;
      }
      return `Selected recipients (${countLabel})`;

    case 'group': {
      const groupId = audienceLike.groupId;
      return groupId ? `Group audience: ${groupId} (${countLabel})` : `Group audience (${countLabel})`;
    }

    case 'event': {
      const eventIds = effectiveEventAudienceIds(audienceLike);
      const names = await resolveEventNames(eventIds);
      if (names.length) {
        return `Event registrants — ${names.join(', ')} (${countLabel})`;
      }
      return `Event registrants (${countLabel})`;
    }

    case 'chat': {
      const chatIds = effectiveChatAudienceIds(audienceLike);
      const names = await resolveChatNames(chatIds);
      if (names.length) {
        return `Chat members — ${names.join(', ')} (${countLabel})`;
      }
      return `Chat members (${countLabel})`;
    }

    case 'location': {
      const locs = effectiveLocationAudienceLabels(audienceLike);
      if (locs.length) {
        return `Members in ${locs.join(', ')} (${countLabel})`;
      }
      return `Members by location (${countLabel})`;
    }

    case 'all_users':
      return `All members (${countLabel})`;

    default:
      return `${MODE_LABELS[mode] || 'Bulk send'} (${countLabel})`;
  }
}

async function resolveCreatedByProfile(authResult) {
  const decoded = authResult.decoded || {};
  let email = String(decoded.email || '').trim() || null;
  let name =
    String(decoded.name || decoded.displayName || '').trim() ||
    null;
  const uid = authResult.uid;
  if (uid && db && (!email || !name)) {
    try {
      const snap = await db.collection('users').doc(uid).get();
      if (snap.exists) {
        const data = snap.data() || {};
        if (!email) email = String(data.email || '').trim() || null;
        if (!name) {
          name =
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const audienceReason = await buildAudienceReason(mode, audienceLike, recipients);
    const createdByProfile = await resolveCreatedByProfile(authResult);
    const storedRecipients = recipients.slice(0, MAX_STORED_RECIPIENTS).map((r) => ({
      email: r.email,
      ...(r.name ? { name: r.name } : {}),
    ...
    const selection = {
      ...
      eventIds: effectiveEventAudienceIds(audienceLike),
      chatId: chatId ?? null,
      chatIds: effectiveChatAudienceIds(audienceLike),
      location: location ?? null,
      locations: effectiveLocationAudienceLabels(audienceLike),
    };
    const rawCampaignData = {
      ...
      recipientsTruncated: recipients.length > MAX_STORED_RECIPIENTS,
      successCount,
      errorCount,
      ...
      createdByName: createdByProfile.name,
