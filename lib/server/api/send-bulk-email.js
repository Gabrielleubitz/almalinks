...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Parse request body
    const { mode, ids, groupId, eventId, chatId, location, subject, html, text, fromName } = req.body;
    ...
    const resolvePayload = { mode, ids, groupId, eventId, chatId, location };
    ...
    console.error('[send-bulk-email] Error:', error?.message || error);
    const isDev = process.env.NODE_ENV === 'development';
    return res.status(500).json({
      ok: false,
      error: isDev ? (error?.message || 'Internal server error') : 'Failed to send bulk email'
    });
  }
}
  ...
  switch (mode) {
    case 'individuals':
      if (!ids || !Array.isArray(ids) || ids.length === 0) {
        throw new Error('ids array is required for individuals mode');
      }
