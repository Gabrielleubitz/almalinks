...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      // Process results (API can return array or single object for 1 recipient)
      const resultsArray = Array.isArray(sendResults) ? sendResults : (sendResults != null ? [sendResults] : []);
      resultsArray.forEach((result, index) => {
        if (result && (result.status === 'sent' || result.status === 'queued')) {
          successCount++;
        } else {
          errorCount++;
          errors.push({
      ...
      for (let i = 0; i < recipients.length; i++) {
        try {
          ...
          if (single && (single.status === 'sent' || single.status === 'queued')) {
            successCount++;
          } else {
            errorCount++;
            errors.push({
              email: recipients[i].email,
              reason: single?.reject_reason || single?.status || 'unknown'
            });
          }
    ...
    // Write audit record to Firestore (omit undefined so Firestore accepts the document)
    const selection = {};
    if (ids != null) selection.ids = ids;
    if (groupId != null) selection.groupId = groupId;
    if (eventId != null) selection.eventId = eventId;
    if (chatId != null) selection.chatId = chatId;
    if (location != null) selection.location = location;
