 ...
 */
/** Normalize Firestore / API event fields before building announcement HTML. */
export function normalizeEventForEmail(event) {
  const raw = event && typeof event === 'object' ? event : {};
  let date = raw.date;
  if (date && typeof date.toDate === 'function') {
    date = date.toDate().toISOString();
  } else if (date && typeof date.toMillis === 'function') {
    date = new Date(date.toMillis()).toISOString();
  } else if (date && typeof date === 'object' && date.seconds != null) {
    date = new Date(Number(date.seconds) * 1000).toISOString();
  } else if (date != null) {
    date = String(date);
  } else {
    date = '';
...
export function eventAnnouncement(event, eventUrl) {
  const normalized = normalizeEventForEmail(event);
  const name = normalized.name || 'Event';
  const dateText = formatEmailDate(normalized.date);
  const location = normalized.location || '';
  const description = normalized.description.trim().replace(/\n/g, '<br/>');
