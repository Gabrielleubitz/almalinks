...
export function welcomeApproved(displayName, loginLink, eventsLink, setupLink) {
  ...
  const signInHelp = setupLink
    ? '<strong>How to sign in:</strong> Use the button above to set your password, then log in at any time with your email address. If you prefer, you can also choose &ldquo;Sign in with Google&rdquo; if you applied with a Google account.'
    : '<strong>How to sign in:</strong> Use the same email you applied with. If you registered with <strong>Google</strong>, choose &ldquo;Sign in with Google&rdquo; on the login page. If you set a <strong>password</strong> when you applied, use that password; if you are unsure, use &ldquo;Forgot password&rdquo; on the login page to set a new one.';
...
<li><strong>Non-political and non-religious</strong> — AlmaLinks is non-political and non-religious; please avoid advocacy threads that would divide the community.</li>
  ...
  const setupBlock = setupLink
    ? `${buildCtaButton(setupLink, 'Set your password \u2192')}
${buildParagraph('This link expires in <strong>1 hour</strong>. After setting your password, use the log-in button below at any time.', { marginBottom: 20 })}`
    : '';
...
${valuesHtml}
${conductHtml}
${buildParagraph(`If you are interested in recommending a prospective AlmaLinks member, ${buildSecondaryLink(criteriaUrl, 'read more about the criteria we look for')}.`, { marginBottom: 12 })}
${buildParagraph(`${buildSecondaryLink(visionUrl, 'Learn more about AlmaLinks vision and programming')}.`, { marginBottom: 16 })}
