...
/** Optional profile/avatar image URL: create a custom Contact property (e.g. URL or single-line text) in HubSpot and set its internal name here. */
const PROFILE_IMAGE_PROPERTY = (typeof process !== 'undefined' && process.env && process.env.HUBSPOT_CONTACT_PROPERTY_PROFILE_IMAGE) ? String(process.env.HUBSPOT_CONTACT_PROPERTY_PROFILE_IMAGE).trim() : '';
...
function buildHubSpotProperties(profile) {
  ...
  const titleSingle = (p.title || p.jobTitle || '').trim();
