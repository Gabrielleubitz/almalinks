 ...
 * Identity: use profile._lookupEmail (canonical DB email for this user) for search; never use
 * request body email alone so one user cannot overwrite another's contact.
 * - If profile.hubspotContactId is set: verify that contact's email matches _lookupEmail; if yes, PATCH; if no, ignore id and fall back to search.
 * - Else: search by _lookupEmail (or profile.email) only; if found, PATCH and return id; if not found, POST create.
 *
 * @param {object} profile - Alma user profile. Use profile._lookupEmail for lookup when set (e.g. current.email from users doc).
...
export async function upsertHubspotContact(profile) {
  ...
  const lookupEmail = (profile?._lookupEmail || profile?.email || '').toString().trim().toLowerCase();
