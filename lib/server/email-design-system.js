...
/** Orange primary CTA for welcome / password-setup emails (Alma gold). */
export function buildOrangeCtaButton(href, label) {
  const url = escapeHtml(href);
  const text = escapeHtml(label);
  const orange = '#FCAF17';
  const orangeHover = '#D89F13';
  return `
<table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation">
  <tr>
    <td align="center" style="padding:24px 0 16px 0;">
      <a href="${url}" target="_blank" rel="noopener noreferrer" style="display:inline-block;padding:16px 36px;background:${orange};color:#1C1C1C !important;font-family:${FONT_FAMILY};font-size:16px;font-weight:700;text-decoration:none;border-radius:8px;border:2px solid ${orangeHover};">${text}</a>
    </td>
  </tr>
</table>`;
}
...
/**
 * Signature line (e.g. "— AlmaLinks Team").
 */
export function buildSignature(text = '— AlmaLinks Team') {
