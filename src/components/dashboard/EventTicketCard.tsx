...
/**
 * Event ticket card - matches Uiverse.io structure (From Uiverse.io by Smit-Prajapati).
 * Same HTML/CSS structure: .card > .top-section (.border, .icons) + .bottom-section (.title, .row .item).
 */
const EventTicketCard: React.FC<EventTicketCardProps> = ({
  ...
  return (
    <div className={`ticket-card ${isExpired ? 'opacity-80' : ''} ${className}`.trim()}>
      ...
      <div className="bottom-section">
        <span className="title">{eventName.toUpperCase()}</span>
        <div className="row row1">
          <div className="item">
            <span className="big-text">{eventDate}</span>
            <span className="regular-text">Date</span>
          </div>
          <div className="item">
            <span className="big-text">{eventTime}</span>
            <span className="regular-text">Time</span>
          </div>
          <div className="item">
            <span className="big-text" title={eventLocation}>{eventLocation}</span>
            <span className="regular-text">Location</span>
          </div>
        </div>
        {attendeeName && (
          <div className="row" style={{ marginTop: 12, paddingTop: 8, borderTop: '1px solid rgba(255,255,255,0.126)' }}>
            <div className="item" style={{ flex: '1 1 100%' }}>
              <span className="big-text">{attendeeName}</span>
              <span className="regular-text">{attendeeEmail}</span>
            </div>
          </div>
        )}
        {ticketId && (
          <div style={{ textAlign: 'center', marginTop: 8, fontSize: 9, color: 'rgba(255,255,255,0.5)' }}>
            #{ticketId}
          </div>
        )}
      </div>
