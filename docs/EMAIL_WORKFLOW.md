...
|----------|----------|-------------|
| `MAILCHIMP_API_KEY` | Yes | Mailchimp API key (format: `xxxxx-us5`). Used for transactional sends and (optionally) Marketing API. |
...
- **API key**: Use `MAILCHIMP_API_KEY` in production env; never commit real keys.
