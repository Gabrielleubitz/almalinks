...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!apiKey) {
      console.warn('⚠️ MANDRILL_API_KEY not configured - skipping admin notification');
      return res.status(200).json({ 
        ok: true, 
        skipped: true,
        reason: 'MANDRILL_API_KEY not configured' 
      });
    }
...
Go to Admin → Pending Registrations to approve/reject.`;
    ...
    console.error('❌ Error in notify-signup:', err);
    return res
      .status(500)
